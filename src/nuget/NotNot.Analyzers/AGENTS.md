...
**Family-wide scan-marker gate (NN_DI_002/003/004/005/006/007)**: every rule whose advice or hazard depends on auto-registration is gated on the relevant type's assembly carrying `[assembly: AutoDiScanAssembly]` — the type is auto-registered ONLY there, so only there is the premise true (NN_DI_002 redundancy, NN_DI_003/005 "add a marker", NN_DI_004 two-path conflict, NN_DI_006 boot crash, NN_DI_007 duplicate scan). Invocation rules gate on the implementation/concrete type's assembly; symbol rules gate on the declaring assembly. **NN_DI_001 is INTENTIONALLY NOT gated** — a lifetime mismatch between an explicit `Add{L}` and the type's own marker is a correctness bug about the developer's stated intent regardless of scan eligibility, so it fires in any assembly.

**NN_DI_007 visibility predicate (runtime-verified)**: scan-visibility keys on the hosted type's OWN declared accessibility (`Type.IsPublic || Type.IsNestedPublic`), NOT effective (enclosing-aware) visibility. A `public` type nested in an `internal` container IS scanned/auto-registered (so an explicit registration of it fires NN_DI_007); a top-level `internal` type is NOT. Proven by `NotNot.Bcl.Core.Tests.AutoDiRuntimeBypassTests.AddNotNotDiServices_PublicNestedInInternal_IsScanned` / `..._TopLevelInternalHostedService_IsNotScanned`. An earlier effective-public predicate produced a false negative on the public-nested-in-internal shape.
