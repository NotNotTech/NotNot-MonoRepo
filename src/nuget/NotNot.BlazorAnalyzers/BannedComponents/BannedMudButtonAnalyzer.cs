...
/// <summary>
/// Detects direct usage of MudButton and MudIconButton in Razor files and recommends NnButton instead.
/// Scans AdditionalTexts registered via the .props file during build.
/// <para>
/// NnButton provides built-in async busy lifecycle (double-click prevention, loading overlay)
/// and is the standard button component for all projects except NnDesign internals.
/// </para>
/// </summary>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public class BannedMudButtonAnalyzer : DiagnosticAnalyzer
{
    ...
    /// <summary>NNB030: Use NnButton instead of MudButton/MudIconButton.</summary>
    public static readonly DiagnosticDescriptor RuleBannedMudButton = new(
        "NNB030",
        "Use NnButton instead of MudButton/MudIconButton",
        "Use NnButton instead of {0}. NnButton provides built-in async busy lifecycle and is the standard button component. (NNB030)",
        Category, DiagnosticSeverity.Warning, isEnabledByDefault: true,
        description: "Direct usage of MudButton and MudIconButton bypasses the async busy lifecycle "
            + "(double-click prevention, loading overlay). Use NnButton which wraps MudButton with "
            + "built-in busy state management.",
        helpLinkUri: HelpBase + "NNB030");
    ...
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
        ImmutableArray.Create(RuleBannedMudButton);
        ...
        string tag, string displayName)
    {
        ...
        while ((pos = text.IndexOf(tag, pos, StringComparison.Ordinal)) >= 0)
        {
            ...
            // This prevents matching <MudButtonGroup when searching for <MudButton.
            ...
            Report(ctx, file.Path, src, pos, tag.Length, RuleBannedMudButton, displayName);
            pos += tag.Length;
        }
    }
    ...
    private static bool IsOptedOut(CompilationAnalysisContext context)
    {
        return context.Options.AnalyzerConfigOptionsProvider.GlobalOptions
                   .TryGetValue("build_property.BannedButtonAnalyzerEnabled", out var value) &&
