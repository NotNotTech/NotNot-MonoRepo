...
public ref struct SpanList<T>
{
   ...
   public static implicit operator Span<T>(SpanList<T> storage) => storage.AsSpan();
   public static implicit operator ReadOnlySpan<T>(SpanList<T> storage) => storage.AsSpan();
}
