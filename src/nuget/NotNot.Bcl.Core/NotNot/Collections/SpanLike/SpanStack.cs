...
public ref struct SpanStack<T>
{
   ...
   public static implicit operator Span<T>(SpanStack<T> storage) => storage.AsSpan();
   public static implicit operator ReadOnlySpan<T>(SpanStack<T> storage) => storage.AsSpan();
}
