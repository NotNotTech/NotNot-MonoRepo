...
/// <summary>
/// Internal version tracker for RefSlotStore_NewSplit instances.
/// Tracks allocations across all instances for debug assistance.
/// </summary>
/// <remarks>
/// This static class provides a global version counter that ensures each new
/// RefSlotStore_NewSplit instance starts with a unique version number.
/// This helps prevent version collisions between different store instances.
/// </remarks>
internal class RefSlotStore_NewSplit_VersionTracker
{
   /// <summary>
   /// Global initial version counter shared across all RefSlotStore_NewSplit instances.
   /// Each new instance increments this to ensure unique starting versions.
   /// </summary>
...
public class RefSlotStore_NewSplit<T> : IDisposeGuard
{
   ...
   public RefSlotStore_NewSplit(int initialCapacity = 10)
   {
      ...
      if (_nextVersion == 0)
      {
         // Skip 0 as it's reserved for unallocated/invalid slots
         _nextVersion = RefSlotStore_NewSplit_VersionTracker._initialVersion++;
      }
