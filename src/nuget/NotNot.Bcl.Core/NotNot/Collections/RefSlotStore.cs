...
public class RefSlotStore<T> : RefSlotStore
{
	...
	public SlotHandle Alloc(ref T data)
	{
		...
		{
			var toReturn = Alloc();
			ref var r_tuple = ref _storage._AsSpan_Unsafe()[toReturn.Index];
			//p_element.handle = handle;
			r_tuple.slotData = data;
	...
	private SlotHandle Alloc()
	{
		lock (_lock)
		{
			...
			ref var r_tuple = ref _storage._AsSpan_Unsafe()[index];
