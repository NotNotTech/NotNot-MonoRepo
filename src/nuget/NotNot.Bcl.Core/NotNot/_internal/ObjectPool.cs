...
public class ObjectPool : IDisposeGuard
{
   ...
   public struct Rented<T> : IDisposable where T : class
   {
      ...
      public Rented(ObjectPool pool, T item, Action<T>? clearAction = null)
      {
         _pool = pool;
         Value = item;
         _clearAction = clearAction;
   ...
   /// <param name="clearAction">optional, for clearing the item before returning to the pool</param>
   /// <returns></returns>
   public Rented<T> Rent<T>(Action<T>? clearAction = null) where T : class, new()
   {
      var item = Get<T>();
      return new Rented<T>(this, item, clearAction);
   }
...
public class StaticPool
{
   ...
   public struct Rented<T> : IDisposable where T : class
   {
      ...
      public Rented(StaticPool pool, T item, Action<T>? clearAction = null)
      {
         _pool = pool;
         Value = item;
         _clearAction = clearAction;
