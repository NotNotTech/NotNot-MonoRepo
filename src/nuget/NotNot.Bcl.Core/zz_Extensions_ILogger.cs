		 ...
		 Span<string> tags = default)
	 {
		  ...
		  lock (_lock)
		  {
				...
				try
				{
					 ...
					 using var argPairOwner = __.pool.GetUsing<List<(string name, object? value)>>(out var argPairs, v => v.Clear());
					 ...
					 {
						  __.Test.Write($"[{level.ToString().ToUpperInvariant()}] {originalMessage}", objToLog0, objToLog1, objToLog2, memberName, sourceFilePath, sourceLineNumber, objToLog0Name, objToLog1Name, objToLog2Name);
					 }
