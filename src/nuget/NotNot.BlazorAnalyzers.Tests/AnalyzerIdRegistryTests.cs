...
public class AnalyzerIdRegistryTests
{
    ...
    public void ManifestoAnalyzers_ExposeTheirDocumentedIds()
    {
        ...
        Assert.Equal("NNB048", NnDesignJsPackageBoundaryAnalyzer.DiagnosticId);
        Assert.Equal("NNB048", NnDesignJsPackageBoundaryAnalyzer.Rule.Id);
        Assert.Equal(DiagnosticSeverity.Error, NnDesignJsPackageBoundaryAnalyzer.Rule.DefaultSeverity);
