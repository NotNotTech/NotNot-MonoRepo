...
/// </summary>
[Inline<Tags>]
public abstract partial class SlimNode : DisposeGuard
{
	...
	{
		_updateCount = currentTick.UpdateTickCount;
	...
	{
		_children._NotNull();
		__.AssertIfNot(_children.Contains(child));
		var childCounter = child._callCounter;
		child.OnRemove();
		__.AssertIfNot(child._callCounter > childCounter, "didn't call base method?");
	...
	public TSingletonNode GetSingletonNode<TSingletonNode>() where TSingletonNode : SlimNode, ISingletonNode
	{
		...
		if (RootNode._NotNull()._singletonCache.TryGetValue(typeof(TSingletonNode), out var node))
		{
