...
/// <summary>
/// The root of the SlimGraph hiearchy.  for an example of how to attach this to your application, for godot see `SlimGraphHost{TRoot}`
/// </summary>
public class RootNode : SlimNode
{
	...
	/// <summary>
	/// Dependency injection container accessible throughout the node tree.
	/// Must be set during construction via object initializer.
	/// <para>available to all SlimNodes via <see cref="SlimNode.MsDIServices"/></para>
	/// </summary>
	public required MsDIContainer MsDIContainer { get; init; }

	public TickState CurrentTick { get; protected set; } = TickState.Default;
	...
	{
		CurrentTick = CurrentTick.ComputeElapse(wallTimeDelta);// TickState.ComputeNext(CurrentTick, wallTimeDelta);
